...
//
{
    ...
    *data                    = track->data;
    ...
    while (pos < count) {
        ...
        uint8_t pat_len  = (uint8_t)(ch_qc_tgen_double() * (UCHAR_MAX - 1)) + 1;
...
//
{
    ...
    ch_msg_init(message);
    message->header_len =
            _ch_tst_gen_data_field(0.1, 0.1, 1024, &message->header);
    message->data_len =
            _ch_tst_gen_data_field(0.1, 0.05, data_count, &message->data);
